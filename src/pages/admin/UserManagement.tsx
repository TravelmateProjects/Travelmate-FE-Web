        ...
        {loading ? (
          ...
          <div style={{ overflowX: 'auto', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
            <table style={{ borderCollapse: 'collapse', width: '100%', background: '#fff', borderRadius: 12, overflow: 'hidden', minWidth: 700 }}>
              ...
              <tbody>
                {users
                  .filter(user => user.account?.role !== 'admin') // Bỏ user có role admin
                  .map(user => (
        ...
        {selectedUser && (
          ...
          >
            ...
            >
              ...
              <h3 style={{ fontWeight: 700, color: '#2F80ED', marginBottom: 18 }}>
                {t('userManagement.detailTitle')}
              </h3>
              {/* Cover Image */}
                ...
                <img
                  src={
                    typeof selectedUser.avatar === 'string'
                      ? selectedUser.avatar
                      : selectedUser.avatar?.url || avatarDefault
                  }
                  alt="Avatar"
                  style={{
                    width: 64,
              ...
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.username')}:</b> {selectedUser.account?.username}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.email')}:</b> {selectedUser.email}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.role')}:</b> {selectedUser.account?.role}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.status')}:</b>{' '}
                <span
                  style={{
                    color: selectedUser.account?.accountStatus ? '#219653' : '#eb5757',
                    fontWeight: 600,
                  }}
                >
                  {selectedUser.account?.accountStatus
                    ? t('userManagement.active')
                    : t('userManagement.inactive')}
                </span>
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.fullName')}:</b> {selectedUser.fullName}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.dob')}:</b> {selectedUser.dob}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.phone')}:</b> {selectedUser.phone}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.address')}:</b> {selectedUser.address}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.hometown')}:</b> {selectedUser.hometown}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.gender')}:</b> {selectedUser.gender}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.cccd')}:</b> {selectedUser.cccd}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.hobbies')}:</b>{' '}
                {selectedUser.hobbies && selectedUser.hobbies.length > 0
                  ? selectedUser.hobbies.join(', ')
                  : ''}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.description')}:</b> {selectedUser.description}
              </div>
              <div style={{ marginBottom: 10 }}>
                <b>{t('userManagement.job')}:</b> {selectedUser.job}
              </div>
